...
Follow the design spec at `specs/argumentation_library_design_spec_batteries_included_python.md` as the source of truth for scope and layout. Core modules live under `arglib/` (core, semantics, io, viz, algorithms, critique, reasoning, ai, cli), schema assets under `arglib/data/`, tests under `tests/`, and docs under `docs/`. Keep new features aligned to the spec layers (core -> semantics -> critique/AI -> integrations).
...
When code changes affect user-facing behavior, update documentation in the same PR:
- `README.md` for top-level capabilities and quick start.
- `docs/` for API and guides (use `uv run mkdocs serve` locally).
- `ROADMAP.md` and `docs/roadmap.md` for milestones.
- The design spec for major behavior changes.
Keep docs and spec in sync with code before pushing.
...
- Types: prefer builtin generics (`list`, `dict`) and `X | None` unions; add hints to public APIs.
...
- Add tests that cover new user-facing behaviors and edge cases.
...
- v0.4 multimodal evidence and long-document mining (split, unify, reconcile graphs).
...
Do not add PDFs in `specs/` to git; keep them local-only.
